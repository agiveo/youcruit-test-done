 ...
 */
const getFiles = (thisPath, otherPath) => {
  ...
  thisFiles.forEach((file) => {
    ...
    const otherMarkdownFile = fs.readFileSync(`${otherPath}/${file}`, 'utf-8')
    const thisFileContents = parseMarkdown(thisMarkdownFile)
    const otherFileContents = parseMarkdown(otherMarkdownFile)

    const date = thisFileContents.date
    const visible = thisFileContents.visible
    const otherVisible = otherFileContents.visible
    const slug = slugify(thisFileContents.title)
    const otherSlug = slugify(otherFileContents.title)

    const obj = { date, slug, visible, otherSlug, otherVisible }
 ...
 */
const writeFileInfo = async (thisPath, otherPath, jsonFile) => {
  const fileArray = await getFiles(thisPath, otherPath)
...
writeFileInfo(
  ...
  path.join(__dirname, 'news', 'sv'),
  path.join(__dirname, 'newsEn.json')
)
writeFileInfo(
  path.join(__dirname, 'news', 'sv'),
  path.join(__dirname, 'news', 'en'),
  path.join(__dirname, 'newsSv.json')
)
writeFileInfo(
  path.join(__dirname, 'calendar', 'en'),
  path.join(__dirname, 'calendar', 'sv'),
  path.join(__dirname, 'calendarEn.json')
)
writeFileInfo(
  path.join(__dirname, 'calendar', 'sv'),
  path.join(__dirname, 'calendar', 'en'),
  path.join(__dirname, 'calendarSv.json')
)
